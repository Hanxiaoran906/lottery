  ...
  methods: {
    ...
    toggle(form) {
      ...
      if (this.running) {
        ...
        const tempRes = luckydrawHandler(
          number,
          allin ? [] : this.allresult,
          num
        );
        console.log('res: ', tempRes);
        const resArr = new Array(47);
        let oddIndex = 0; // 奇数
        let evenIndex = 1; // 偶数
        const len = tempRes.length - 1;
        for (let i = len; i > 0; i--) {
          if (this.blackList.indexOf(tempRes[i]) > -1) {
            // 如果在黑名单里，则放到偶数位，偶数索引加2
            resArr[evenIndex] = tempRes[i];
            evenIndex += 2;
            tempRes.splice(i, 1);
          }
          if (this.whiteList.indexOf(tempRes[i]) > -1) {
            // 如果在白名单里，则放在奇数位，奇数索引加2
            resArr[oddIndex] = tempRes[i];
            oddIndex += 2;
            tempRes.splice(i, 1);
          }
        }
        ...
        //   console.log(item)
        //   if (!item) {
        //     item = tempRes[remainIdx];
        //     remainIdx++;
        //   }
        // });
        console.log('tempRes: ', tempRes, 'resArr: ', resArr);
        this.resArr = resArr;
